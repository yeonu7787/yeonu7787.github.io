...
window.BlogView = (() => {
 ...
 async function get(query){
  const r=await fetch(cfg.url+"/rest/v1/posts?"+query,{headers:{apikey:cfg.key}});
  if(!r.ok)throw new Error("게시글을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.");
  return r.json();
 }
 async function imageUrl(path){
  const r=await fetch(cfg.url+"/storage/v1/object/sign/post-images/"+path.split("/").map(encodeURIComponent).join("/"),{method:"POST",headers:{apikey:cfg.key,"Content-Type":"application/json"},body:JSON.stringify({expiresIn:60})});
 ...
 async function listing(root){
  ...
  try{
   if(id){
    const rows=await get("select=*&published=eq.true&id=eq."+encodeURIComponent(id)+"&limit=1"),p=rows[0];
    if(!p){root.innerHTML='<h1>글을 찾을 수 없습니다.</h1><a class="text-link" href="/blog/">Blog 목록으로</a>';return;}
    document.title=p.title+" · 이연우";
    root.innerHTML='<article class="post-detail"><a class="text-link" href="/blog/">← Blog 목록으로</a><p class="muted">'+esc(date(p.created_at))+'</p><h1>'+esc(p.title)+'</h1><div class="prose">'+esc(p.content)+'</div><div class="post-gallery">'+(p.images||[]).map(path=>'<figure data-photo="'+esc(path)+'"></figure>').join("")+'</div></article>';
   }else{
    const page=Math.max(0,Math.floor(Number(params.get("page"))||0));
    const rows=await get("select=*&published=eq.true&order=created_at.desc,id.desc&limit=12&offset="+page*12);
    root.innerHTML='<div class="eyebrow">Personal notes</div><h1>Blog</h1>'+(rows.length?'<div class="post-grid">'+rows.map(card).join("")+'</div>':'<p>아직 공개된 글이 없습니다.</p>')+
